...
const saveGameData = (userId, score, genre, year, hipster) => {
    return new Promise((resolve, reject) => {
        // score가 빈 값일 경우 처리 (NaN이 되지 않도록)
        const parsedScore = score ? parseInt(score, 10) : null;

        const query = 'INSERT INTO game_data (userId, score, genre, year, hipster) VALUES (?, ?, ?, ?, ?)';
        
        db.query(query, [userId, parsedScore, genre, year, hipster], (err, result) => {
            if (err) {
