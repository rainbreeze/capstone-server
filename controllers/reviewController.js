...
const createReview = (req, res) => {
    ...
    const { user_id, playlist_music_id, album_image_url, genre, rating, comment, playlist_music_name, user_name } = req.body;
    console.log(user_id, playlist_music_id, album_image_url, genre, rating, comment, playlist_music_name, user_name);
    if (!user_id || !playlist_music_id || !rating) {
        return res.status(400).json({ message: '필수 항목이 누락되었습니다.' });
    }

    reviewModel.saveReview(user_id, playlist_music_id, album_image_url, genre, rating, comment, playlist_music_name, user_name, (err, result) => {
        if (err) {
